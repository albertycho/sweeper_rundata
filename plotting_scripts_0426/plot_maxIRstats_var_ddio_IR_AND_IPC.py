...
#in var_ddio, partition is just ddio ways
...
#
#color_list[1][getindex('clean',ddio_setups)]=color_list[1][0] #IPC
#color_list[1][getindex('nocl',ddio_setups)]=color_list[1][0]   #IPC
...
if infile in os.listdir('.'):
    ...
    while line:
        ...
        if(ir_in_Gbps > max_ir):
            max_ir=ir_in_Gbps
            max_ir_cfg=pts+'ways '+ds
            max_ir_cfg=max_ir_cfg.replace('clean','Sweeper')
        if((float(tmp[12])) > max_ipc):
            max_ipc = float(tmp[12])
            max_ipc_cfg=pts+'ways '+ds
            max_ipc_cfg=max_ipc_cfg.replace('clean','Sweeper')
    ...
    iax2.set_ylabel('Normalized to IPC\n@ '+str(max_ipc_cfg))
    #iax2.set_ylabel('Normalized\nX-MEM IPCs')



    ifig.savefig('var_ddio_IR_IPC'+'.png',bbox_inches='tight')
