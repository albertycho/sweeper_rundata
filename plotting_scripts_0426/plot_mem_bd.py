...
if infile in os.listdir('.'):
    ...
    while line:
        ...
        toparr[psi][rbci][mci][dsi].append(float(tmp[1])/packet_count)   ##0 app_dirty_ev
        toparr[psi][rbci][mci][dsi].append(float(tmp[2])/packet_count)   ##1 rb_dirty_ev
        toparr[psi][rbci][mci][dsi].append(float(tmp[3])/packet_count)   ##2 lb_dirty_ev
        toparr[psi][rbci][mci][dsi].append(float(tmp[4])/packet_count)   ##3 NF_app_access
        toparr[psi][rbci][mci][dsi].append(float(tmp[6])/packet_count)   ##4 rb_access
        toparr[psi][rbci][mci][dsi].append(float(tmp[7])/packet_count)   ##5 lb_access
        #### repositioning memhog to be at the end
        toparr[psi][rbci][mci][dsi].append(float(tmp[5])/packet_count)   ##6 memhog_access
    ...
    for psi in range(pslen):
        for rbi in range(rblen):
            ...
            for mci in range(mclen): 
                ...
                #ifig.savefig(str(packet_sizes[psi])+'_'+str(rb_counts[rbi])+'_'+str(mcs[mci]), dpi=1000)
