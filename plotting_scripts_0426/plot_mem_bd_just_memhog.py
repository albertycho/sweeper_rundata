...
parser.add_argument('--outdir', type=str, default='mem_bd_plots_just_memhog')  
    ...
    for psi in range(pslen):
        for rbi in range(rblen):
            ...
            for mci in range(mclen): 
                ...
                barwidth = 0.5
                iax.bar(X_axis, tmparr[6], edgecolor='black',alpha=0.8,color=color_list[6], label=field_names[6], width=barwidth)
                #botarr = tmparr[0];
