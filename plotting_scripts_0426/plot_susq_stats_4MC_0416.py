...
parser.add_argument('--infile', type=str, default='leakyDMA_stats_4MC.csv')  
parser.add_argument('--outdir', type=str, default='leakyDMA_plots_4MC')  
...
color_list[getindex('4',mcs)][getindex('2',ddio_setups)]='red'
color_list[getindex('4',mcs)][getindex('12',ddio_setups)]='orange'
color_list[getindex('4',mcs)][getindex('clean',ddio_setups)]='green'
color_list[getindex('4',mcs)][getindex('clean12',ddio_setups)]='blue'
    ...
    for i in range(len(field_names)):
        ...
        for j in (range(mclen)):
            for k in range(len(ddio_setups)):
                ...
                iax.bar(X_axis-0.2+(xoffset), tmparr[j][k],edgecolor='black', alpha=0.6, color=color_list[j][k], label=str(ddio_setups[k])+"ways", width=barwidth)

        plt.xticks(X_axis, xtick_labels)
        iax.legend(bbox_to_anchor=(1,1) , ncol=1)
