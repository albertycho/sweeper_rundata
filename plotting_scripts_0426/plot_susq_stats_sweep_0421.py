...
if infile in os.listdir('.'):
    ...
    for i in range(len(field_names)):
        ...
        if ('MBW' in field_names[i]) or ('NIC_RB_Miss_Rate' in field_names[i]): 
            ...
            for k in (range(len(ddio_setups))):
                ...
                for j in range(len(ways)):
                    ...
                    iax.bar(X_axis-barwidth*1.5+(xoffset), tmparr[j][k],edgecolor='black', alpha=alval, hatch='', color=color_list[j+(k*wlen)], label=label_name, width=barwidth, zorder=zorderval)
        ...
        iax.legend(bbox_to_anchor=(0.5,1.2) , ncol=2, loc='center', columnspacing=0.6)#, columnspacing=)
        plt.setp(iax.get_xticklabels(), horizontalalignment='center')
        #ifig.set_size_inches(16,3)
        ifig.set_size_inches(20,21)
        ...
        ifig.savefig('TLD_'+field_names[i]+'_sweep.pdf', bbox_inches='tight')
