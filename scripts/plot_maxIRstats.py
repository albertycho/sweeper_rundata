...
nic_rb_mr.append(nic_rb_mr_3con)
...
if infile in os.listdir('.'):
    ...
    while line:
        ...
        if '3con' in cons:
            setups_3con.append(su)
            #max_ir_3con.append(float(tmp[1]))
            max_ir_3con.append(ir_in_Gbps)
            mbw_3con.append(float(tmp[2]))
            #avg_st_3con.append(float(tmp[3]))
            avg_st_3con.append(st_in_ns)
            nic_rb_mr_3con.append(float(tmp[6]))
        ...
        if 'ideal' in cons:
            ...
            wr_lat_ideal.append(float(tmp[10])) #these two will be 0 for simp
            rd_lat_ideal.append(float(tmp[11]))
...
#    for i in range(1,len(field_names)):
    ...
    for i in {1,2,3,6,10 ,11}:
        ...
        iax.bar(X_axis+0.15, arrptr[i-1][2],edgecolor='black', alpha=0.5, color="red", label='3mc: 154Gbps', width=0.1)
