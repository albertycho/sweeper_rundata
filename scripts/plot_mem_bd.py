...
color_list[getindex('rb_dirty_evct' ,field_names    )]  ='blue'
...
color_list[getindex('rb_access'     ,field_names    )]  ='darkred'
...
if infile in os.listdir('.'):
    ...
    while line:
        ...
        toparr[psi][rbci][mci][dsi].append(float(tmp[5]))   ##4 memhog_access
        toparr[psi][rbci][mci][dsi].append(float(tmp[6]))   ##5 rb_access
        toparr[psi][rbci][mci][dsi].append(float(tmp[7]))   ##6 lb_access
            ...
            for mci in range(1,mclen): 
                ...
                for dsi in range(ddioslen):
                    for j in range(len(field_names)):
                        tmparr[j].append(toparr[psi][rbci][mci][dsi][j])


                ifig,iax=plt.subplots()
                iax.set_title(str(psi)+'_'+str(rbi)+'_'+str(mci))
                ...
                for j in range(1,len(field_names)):
                    iax.bar(X_axis, tmparr[j], bottom=tmparr[j-1], edgecolor='black',alpha=0.8,color=color_list[j], label=field_names[j], width=barwidth)
